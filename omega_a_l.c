...
/* グローバル変数（Omega相座標データ: 22原子） */
double original_position[22][3] = {
/*1*/   {0, 0, 0},
/*2*/   {-2.27819362, -3.945947, 0},
/*3*/   {2.27819362, -3.945947, 0},
/*4*/   {4.55638737, 0, 0},
/*5*/   {2.27819368, 3.94594713, 0},
/*6*/   {-2.27819369, 3.94594713, 0},
/*7*/   {-4.55638724, 0, 0},
/*8*/   {2.25E-06, -2.63063263, 1.40600179},
/*9*/   {2.27819136, -1.31531436, 1.40600179},
/*10*/  {2.27819601, 1.31531437, 1.40600179},
/*11*/  {-2.33E-06, 2.63063276, 1.40600179},
/*12*/  {-2.27819137, 1.31531437, 1.40600179},
/*13*/  {-2.27819588, -1.31531436, 1.40600179},
/*14*/  {0, 0, 2.81200358},
/*15*/  {-2.27819362, -3.945947, 2.81200358},
/*16*/  {2.27819362, -3.945947, 2.81200358},
/*17*/  {4.55638737, 0, 2.81200358},
/*18*/  {2.27819368, 3.94594713, 2.81200358},
/*19*/  {-2.27819369, 3.94594713, 2.81200358},
/*20*/  {-4.55638724, 0, 2.81200358},
/*21*/  {2.25E-06, -2.63063263, 4.21800537},
/*22*/  {-4.55638724, -2.63063263, 1.40600179},
};

/* Seitz Operator Database (Omega相用: 24操作) */
...
void affine_transform(double R[4][4], double axis[3], double n,
    ...
    if (norm < EPS) {
        ...
        /* 注意: ここで配列を書き換えるため、呼び出し側はstructのコピーを渡す必要がある */
        translation[0] *= A_X;
        translation[1] *= A_Y;
        translation[2] *= A_Z;
...
void calculate_alpha_l(double position[][3], int points[], int ATOM, FILE *fp) {
    fprintf(fp, "\n=== Calculation of Multiplicity (alpha_l) for Omega Phase ===\n");
    printf("\n=== Calculation of Multiplicity (alpha_l) for Omega Phase ===\n");
    ...
    for(int k=0; k<NUM_OPE; k++) {
        /* 重要: affine_transform内でtranslationを書き換えるため、structをコピーして渡す */
...
int main(void) {
    char filename[256];
    printf("出力ファイル名を入力してください（例: omega_alpha_result.txt）：");
    scanf("%255s", filename);
    ...
    scanf("%d", &ATOM);
    if(ATOM > 22) {
        printf("原子数が多すぎます（Max 22）。処理を中断します。\n");
        fclose(fp);
        return 1;
    }
    ...
    printf("各原子の番号を入力してください（Omega相は 1-22 まで）。\n");
    
    for (int i=0; i<ATOM; i++) {
        printf("Atom %d No: ", i+1);
        scanf("%d", &points[i]);
        if (points[i] < 1 || points[i] > 22) {
